...
class Trainer(object):
    ...
    def train_epoch(self):
        ...
        for i, (HLA, Antigen, labels) in enumerate(tqdm(self.train_dataloader)):
            self.step += 1
            HLA, Antigen, labels = HLA.to(self.device), Antigen.to(self.device), labels.float().to(self.device)
            self.optim.zero_grad()
            HLA, Antigen, f, score = self.model(HLA, Antigen)
            if self.n_class == 1:
    ...
    def train_da_epoch(self):
        ...
        num_batches = len(self.train_dataloader)
        for i, (batch_s, batch_im) in enumerate(tqdm(self.train_dataloader)):
            self.step += 1
            HLA, Antigen, labels = batch_s[0].to(self.device), batch_s[1].to(self.device), batch_s[2].float().to(
                self.device)
            HLA = torch.Tensor(HLA)
            Antigen = torch.Tensor(Antigen)
            HLA_im, Antigen_im = batch_im[0].to(self.device), batch_im[1].to(self.device)
            HLA_im = torch.Tensor(HLA_im)
            Antigen_im = torch.Tensor(Antigen_im)
            self.optim.zero_grad()
            ...
            if self.current_epoch >= self.da_init_epoch:
                HLA_im, Antigen_im, f_im, im_score = self.model(HLA_im, Antigen_im)
                if self.da_method == "CDAN":
                    ...
                    if self.original_random:
                        ...
                        adv_output_an_score = self.domain_dmm(random_out.view(-1, random_out.size(1)))
                    else:
                        ...
                        if self.random_layer:
                            ...
                            adv_output_an_score = self.domain_dmm(random_out)
                        else:
                            adv_output_an_score = self.domain_dmm(feature)

                    reverse_f_im = ReverseLayerF.apply(f_im, self.alpha)
                    softmax_output_im = torch.nn.Softmax(dim=1)(im_score)
                    softmax_output_im = softmax_output_im.detach()
                    # reverse_output_im = ReverseLayerF.apply(softmax_output_im, self.alpha)
                    if self.original_random:
                        random_out_im = self.random_layer.forward([reverse_f_im, softmax_output_im])
                        adv_output_im_score = self.domain_dmm(random_out_im.view(-1, random_out_im.size(1)))
                    else:
                        feature_im = torch.bmm(softmax_output_im.unsqueeze(2), reverse_f_im.unsqueeze(1))
                        feature_im = feature_im.view(-1, softmax_output_im.size(1) * reverse_f_im.size(1))
                        if self.random_layer:
                            random_out_im = self.random_layer.forward(feature_im)
                            adv_output_im_score = self.domain_dmm(random_out_im)
                        else:
                            adv_output_im_score = self.domain_dmm(feature_im)

                    if self.use_da_entropy:
                        entropy_an = self._compute_entropy_weights(score)
                        entropy_im = self._compute_entropy_weights(im_score)
                        src_weight = entropy_an / torch.sum(entropy_an)
                        tgt_weight = entropy_im / torch.sum(entropy_im)
                    else:
                        src_weight = None
                        tgt_weight = None

                    n_an, loss_cdan_an = cross_entropy_logits(adv_output_an_score, torch.zeros(self.batch_size).to(self.device),
                                                                src_weight)
                    n_im, loss_cdan_im = cross_entropy_logits(adv_output_im_score, torch.ones(self.batch_size).to(self.device),
                                                                tgt_weight)
                    da_loss = loss_cdan_an + loss_cdan_im
                else:
    ...
    def test(self, dataloader="test"):
        ...
        with torch.no_grad():
            self.model.eval()
            for i, (HLA, Antigen, labels) in enumerate(data_loader):
                HLA, Antigen, labels = HLA.to(self.device), Antigen.to(self.device), labels.float().to(self.device)
                if dataloader == "val":
                    HLA, Antigen, f, score = self.model(HLA, Antigen)

                elif dataloader == "test":
                    HLA, Antigen, f, score = self.best_model(HLA, Antigen)
                if self.n_class == 1:
                    n, loss = binary_cross_entropy(score, labels)
                else:
                    n, loss = cross_entropy_logits(score, labels)
