...
class HLAIImaster(nn.Module):
    def __init__(self, **config):
        super(HLAIImaster, self).__init__()
        self.bcn = weight_norm(
            BANLayer(v_dim=34, q_dim=25, h_dim=128, h_out=2),
            name='h_mat', dim=None)
        self.HLA_emb = nn.Linear(21, 128)
        self.Anti_emb = nn.Linear(21, 128)

        self.HLA_CNN = CNN(32,1,34)
        self.Anti_CNN = CNN(32,1,25)
        self.layers = 3
        self.hidden_size = config['Trans']['EMB_SIZE']
        self.intermediate_size = config['Trans']['Intermediate_size']
        self.num_attention_heads = config['Trans']['Num_attention_heads']
        self.attention_probs_dropout_prob = config['Trans']['Attention_probs_dropout_prob']
        self.hidden_dropout_prob = config['Trans']['Hidden_dropout_prob']
        self.out_binary = config['DECODER']['BINARY']
        ...
        self.mlp_classifier = MLPDecoder(128, 64, 32, binary=self.out_binary)

    def forward(self, HLA, Antigen, mode="train"):
        # print("HLA:", HLA)
        HLA = self.HLA_emb(HLA)
        # HLA, HLA1, HLA2 = self.HLA_CNN(HLA)
        HLA = self.encoder(HLA)
        # print("shape:", HLA.shape)
        HLA = HLA.permute(0, 2, 1)

        Antigen = self.Anti_emb(Antigen)
        # Antigen, Antigen1, Antigen2 = self.Anti_CNN(Antigen)
        ...
        if mode == "train":
            return HLA, Antigen, f, score
        elif mode == "eval":
            return HLA, Antigen, score, att
